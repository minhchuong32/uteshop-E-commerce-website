...
<body class="bg-light">
	<div class="container mt-4">
		...
		<c:if test="${not empty sessionScope.message}">
			<div
				class="alert alert-success alert-dismissible fade show alert-download"
				role="alert">
				...
				<hr>
				<p class="mb-2">
					<i class="bi bi-file-earmark-pdf-fill me-1"></i> <strong>File:</strong>
					${sessionScope.fileName}
				</p>
				<div class="d-grid gap-2">
					<!-- Link tải PDF -->
					<a href="${sessionScope.downloadLink}"
						class="btn btn-primary btn-sm" download> <i
						class="bi bi-download me-1"></i>Tải xuống PDF
					</a>
					<!-- Link xem trước PDF -->
					<a href="${sessionScope.downloadLink}"
						class="btn btn-outline-primary btn-sm" target="_blank"> <i
						class="bi bi-eye me-1"></i>Xem trước
					</a>
				</div>
		...
		<c:if test="${not empty sessionScope.error}">
			<div
				class="alert alert-danger alert-dismissible fade show alert-download"
				role="alert">
		...
		<h3 class="fw-bold text-primary-custom mb-3">📦 Quản lý vận
			chuyển</h3>
						...
						<c:forEach var="d" items="${deliveries}">
							<tr>
								...
								<td class="text-center"><span
									class="badge bg-${d.status == 'Delivered' ? 'success' : d.status == 'In Transit' ? 'warning' : 'secondary'}">
										${d.status} </span></td>
								<td class="text-center"><c:choose>
										<c:when test="${not empty d.deliveryNote}">
											...
											<a href="${pageContext.request.contextPath}${d.deliveryNote}"
												class="btn btn-outline-success btn-sm rounded-pill" download>
												<i class="bi bi-download me-1"></i>Tải
											</a>
											<a href="${pageContext.request.contextPath}${d.deliveryNote}"
												class="btn btn-outline-primary btn-sm rounded-pill"
												target="_blank"> <i class="bi bi-eye me-1"></i>Xem
											</a>
										</c:when>
										<c:otherwise>
									...
									</c:choose> <c:if test="${not empty d.noteText}">
				...
				<div class="modal-body">
					...
					<div class="mb-3">
						...
						<textarea class="form-control" id="noteText" name="noteText"
							rows="3" placeholder="Nhập ghi chú cho phiếu giao hàng..."></textarea>
					</div>
				</div>
		...
		<div class="modal-dialog modal-dialog-centered">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<p class="text-danger small mb-0">⚠️ File PDF đính kèm cũng sẽ
						bị xóa!</p>
				</div>
				<div class="modal-footer">
					<button type="button"
