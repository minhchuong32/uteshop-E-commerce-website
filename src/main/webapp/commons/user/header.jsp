			...
			<div class="collapse navbar-collapse" id="navbarContent">
				...
				<ul class="navbar-nav me-3">
					<li class="nav-item"><a
						...
						href="${pageContext.request.contextPath}/user/home#main">Trang
							chủ</a></li>
					<li class="nav-item"><a
						class="nav-link nav-link-header text-dark fw-semibold"
						href="${pageContext.request.contextPath}/user/home#product">Sản
							phẩm</a></li>
					<li class="nav-item"><a
						class="nav-link nav-link-header text-dark fw-semibold"
						href="${pageContext.request.contextPath}/user/home#category">Danh
							mục</a></li>
					<li class="nav-item"><a
						class="nav-link nav-link-header text-dark fw-semibold"
				...
				<c:set var="searchAction"
					value="${not empty shop 
           ? pageContext.request.contextPath.concat('/user/shop/detail') 
           : pageContext.request.contextPath.concat('/user/home')}" />
				<form class="d-flex mx-auto me-3 search-form"
					action="${searchAction}" method="get"
					style="max-width: 400px; width: 100%;">

					<%-- Add hidden ID field only if on a shop page --%>
				...
				<ul class="navbar-nav align-items-center me-3">
					<li class="nav-item dropdown position-relative"><a
						...
						role="button" data-bs-toggle="dropdown" aria-expanded="false">
							<i class="bi bi-bell-fill fs-5 text-primary-custom"></i> <c:if
								test="${not empty notifications && fn:length(notifications) > 0}">
								<span
									class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"
									style="font-size: 0.7rem; padding: 3px 6px;">
									${fn:length(notifications)} </span>
							</c:if>
						...
						<ul class="dropdown-menu dropdown-menu-end shadow"
							...
							<c:choose>
								<c:when test="${not empty notifications}">
									<c:forEach var="n" items="${notifications}">
										<li><a class="dropdown-item small text-wrap"
											href="${pageContext.request.contextPath}/user/notifications/view?id=${n.id}">
												<div class="d-flex align-items-start">
													...
													<div>
														<strong class="text-dark"> Khiếu nại
															#${n.relatedComplaint.complaintId} </strong><br> ${n.message}<br>
								...
								<c:otherwise>
									<li>
										<p class="dropdown-item text-muted small mb-0">Không có
											thông báo mới.</p>
									</li>
								</c:otherwise>
							</c:choose>
							...
							<li><hr class="dropdown-divider"></li>
							<li><a
								class="dropdown-item text-center text-primary-custom fw-semibold"
