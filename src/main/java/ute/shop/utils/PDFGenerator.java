...
public class PDFGenerator {
    ...
    public static void generateRevenueReport(OutputStream out,
        ...
        Paragraph title = new Paragraph("BÁO CÁO DOANH THU SÀN THƯƠNG MẠI", titleFont);
        title.setAlignment(Element.ALIGN_CENTER);
        ...
        Paragraph info = new Paragraph(String.format(
                "Tỷ lệ chiết khấu: %.0f%%\nPhí sàn: %,d ₫\nDoanh thu sau chiết khấu: %,d ₫",
                feeRate.doubleValue(),
                platformFee.longValue(),
        ...
        table.addCell(createHeaderCell("Tháng"));
        table.addCell(createHeaderCell("Doanh thu (₫)"));

        for (Object[] row : revenueByMonth) {
            table.addCell(createNormalCell(String.valueOf(row[0])));
            table.addCell(createNormalCell(String.format("%,d", ((Number) row[1]).longValue())));
        }
        document.add(table);

        // --- Phân tích tự động ---
        Paragraph analysisTitle = new Paragraph("\nBáo cáo phân tích:", new Font(Font.FontFamily.HELVETICA, 13, Font.BOLD));
        analysisTitle.setSpacingBefore(15);
        document.add(analysisTitle);

        Paragraph analysisText = new Paragraph(analysis, new Font(Font.FontFamily.HELVETICA, 12));
        analysisText.setSpacingBefore(5);
        document.add(analysisText);

        // --- Chân trang ---
        Paragraph footer = new Paragraph("\nBáo cáo tự động tạo bởi hệ thống UteShop © 2025",
