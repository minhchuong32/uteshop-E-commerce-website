...
//
//			double avgFixed = list.stream().filter(p -> "fixed".equals(p.getDiscountType())).map(Promotion::getValue)
...
public class PromotionController extends HttpServlet {
    ...
    private IPromotionService promoService = new PromotionServiceImpl();
    private IShopService shopService = new ShopServiceImpl();
    private IProductService productService = new ProductServiceImpl(); // ✅ thêm service sản phẩm
