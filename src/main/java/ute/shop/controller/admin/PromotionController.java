...
public class PromotionController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			if (uri.endsWith("/admin/promotions")) {
				...
				double avgPercent = list.stream().filter(p -> "percent".equals(p.getDiscountType()))
						.map(Promotion::getValue).mapToDouble(BigDecimal::doubleValue).average().orElse(0.0);

				double avgFixed = list.stream().filter(p -> "fixed".equals(p.getDiscountType()))
						.map(Promotion::getValue).mapToDouble(BigDecimal::doubleValue).average().orElse(0.0);
				...
				resp.sendRedirect(req.getContextPath() + "/admin/promotions?message=DelSuccess");
			}
		} catch (Exception e) {
			e.printStackTrace();
			resp.sendRedirect(req.getContextPath() + "/admin/promotions?error=errorGet");
		}
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			Integer productId = (productParam != null && !productParam.isEmpty()) ? Integer.parseInt(productParam)
					: null;
			...
			resp.sendRedirect(req.getContextPath() + "/admin/promotions?message=success");
		} catch (Exception e) {
			e.printStackTrace();
			resp.sendRedirect(req.getContextPath() + "/admin/promotions?error=errorPost");
		}
	}
}
