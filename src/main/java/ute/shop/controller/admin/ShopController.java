...
public class ShopController extends HttpServlet {
        ...
        if (uri.endsWith("/shops")) {
            ...
            if (idParam != null && !idParam.isEmpty()) {
                try {
                    ...
                    req.getSession().setAttribute("error", "ID cửa hàng không hợp lệ!");
                }
            }
            resp.sendRedirect(req.getContextPath() + "/admin/shops");
        }
    ...
    // Xử lý thêm cửa hàng
    // ==========================
    private void handleAddShop(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (filePart != null && filePart.getSize() > 0) {
            ...
            String uploadPath = req.getServletContext()
                    .getRealPath("/assets/images/shops");
        ...
        req.getSession().setAttribute("success", "Thêm cửa hàng mới thành công!");
        resp.sendRedirect(req.getContextPath() + "/admin/shops");
    }
    ...
    // ==========================
    private void handleEditShop(HttpServletRequest req, HttpServletResponse resp)
            throws IOException, ServletException {
        ...
        req.getSession().setAttribute("success", "Cập nhật cửa hàng thành công!");
        resp.sendRedirect(req.getContextPath() + "/admin/shops");
    }
